...
## 简介
本文件面向BokAgent的多数据库支持能力，系统化梳理当前实现中对PostgreSQL与MySQL的差异处理、JSON字段兼容策略、数据类型映射、事务管理现状与限制、部署最佳实践以及数据迁移与同步方案建议。当前代码库已明确包含PostgreSQL工作流数据库与MySQL业务数据库的容器编排与基础驱动依赖，并通过MyBatis-Plus与自定义类型处理器实现JSON字段的序列化/反序列化。MySQL作为业务数据库存在，但未在后端代码中发现直接的MySQL数据源配置与实体映射；因此，本文在“现状”部分严格基于仓库内容，在“建议”部分提供可落地的多数据库演进建议。
...
APP["Spring Boot 应用"]
MP["MyBatis-Plus"]
JT["JsonbTypeHandler<br/>JSONB类型处理器"]
end
subgraph "数据库"
...
- JSON字段处理：通过自定义JsonbTypeHandler将Java对象序列化为PostgreSQL JSONB类型；实体类中对JSON字段标注了类型处理器。
...
Handler->>DB : 将Java对象序列化为JSONB并入库
DB-->>Mapper : 返回结果集
...
### JSON字段处理与兼容性
- PostgreSQL JSONB类型
  - 自定义JsonbTypeHandler负责将Java对象序列化为JSONB并入库，读取时反序列化回Java对象。
  - 实体类中对graphData、inputData、outputData字段标注了类型处理器。
- MySQL JSON类型
  - 当前代码库未针对MySQL JSON类型进行专门处理；若启用MySQL，需新增类型处理器或采用字符串字段配合Jackson序列化。
- 兼容性策略建议
  - 若需跨数据库统一，可在实体层统一使用字符串字段并由服务层完成序列化/反序列化，避免数据库特定类型耦合。
...
- 字符串类型
  - PostgreSQL使用VARCHAR/TEXT；MySQL使用VARCHAR/TEXT/JSON。当前JSON字段在PostgreSQL使用JSONB，在MySQL需另行处理。
- 跨库兼容建议
...
## 结论
当前BokAgent已具备PostgreSQL工作流数据库的完整链路：容器编排、迁移脚本、实体映射与JSONB类型处理。MySQL作为业务数据库已在容器中可用，但后端尚未启用其数据源。建议在保持现有PostgreSQL能力的同时，按“统一JSON序列化、按需启用MySQL、完善迁移与监控”的路径推进多数据库演进，确保平滑过渡与长期可维护性。
...
- 字符串类型：VARCHAR/TEXT一致；JSON字段在PostgreSQL使用JSONB，在MySQL建议使用TEXT+Jackson序列化
