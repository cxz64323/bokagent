...
## 更新摘要
**变更内容**
- 更新了Spring AI依赖状态说明，明确指出LLM相关依赖当前被注释且服务处于非功能性状态
- 新增了依赖注释说明和配置要求章节
- 更新了故障排查指南，增加了Spring AI配置相关的故障排查项
- 补充了依赖启用指南，说明如何正确配置和启用Spring AI功能
...
**重要更新**：当前Spring AI相关依赖（LLM功能）处于注释状态，服务暂不可用，需要在完成相应配置后才能启用。
...
I["LLM服务<br/>LLMService.java"] --> A
J["工作流引擎<br/>WorkflowEngine.java"] --> A
```
...
- [backend/src/main/java/com/bokagent/service/LLMService.java:1-73](file://backend/src/main/java/com/bokagent/service/LLMService.java#L1-L73)
...
**重要说明**：当前LLM功能由于Spring AI依赖被注释而不可用，系统将自动降级到基础工作流执行模式。

```mermaid
graph TB
subgraph "后端服务"
SBA["Spring Boot 应用<br/>BokAgentApplication.java"]
SAI["Spring AI<br/>OpenAI/Deepseek/Qwen<br/>当前不可用"]
...
**图表来源**
- [backend/src/main/java/com/bokagent/service/LLMService.java:13-14](file://backend/src/main/java/com/bokagent/service/LLMService.java#L13-L14)
...
- AI 与大模型
  - **spring-ai-openai-spring-boot-starter：OpenAI/Deepseek/Qwen 多厂商接入（当前注释，待启用）**
- ORM 与数据库
...
- 存储与对象
  - minio：对象存储客户端
- 图计算与工作流
  - **langgraph4j-core：工作流状态机与检查点（当前注释，待启用）**
- 开发与工具
...
**重要更新**：Spring AI 和 LangGraph4J 依赖当前被注释，需要在配置完成后才能启用。注释中包含了启用说明和仓库要求。
...
- **重要**：Spring AI 配置部分（OpenAI、Deepseek、Qwen）已准备好，只需启用对应依赖即可生效

**章节来源**
- [backend/src/main/resources/application.yml:16-190](file://backend/src/main/resources/application.yml#L16-L190)

### 依赖注释说明与配置要求
当前项目中存在两处关键依赖被注释，需要特别注意：

1. **Spring AI OpenAI Starter 注释说明**：
   ```xml
   <!-- Spring AI - 暂时注释，等待正式版本发布 -->
   <!--
   <dependency>
   ...
   -->
   ```
   - 需要在 dependencyManagement 中启用 Spring AI BOM
   - 需要确保 Spring Milestones 仓库可用
   - 配置完成后，LLMService 将恢复正常功能

2. **LangGraph4J 注释说明**：
   ```xml
   <!-- LangGraph4J - 暂时注释，需要添加到自定义仓库 -->
   <!--
   <dependency>
   ...
   -->
   ```
   - 需要添加自定义仓库以获取 LangGraph4J
   - 配置完成后，可切换到 LangGraph4J 引擎

**章节来源**
- [backend/pom.xml:51-58](file://backend/pom.xml#L51-L58)
- [backend/pom.xml:98-105](file://backend/pom.xml#L98-L105)

### LLM服务状态说明
LLMService 当前处于非功能性状态，具有以下特点：

- **状态标识**：服务类注释明确标注"Spring AI 依赖暂时被注释，此服务暂不可用"
- **降级行为**：当调用 chat 方法时，会返回警告信息而非实际的LLM调用
- **配置准备**：application.yml 中的 Spring AI 配置（API Key、Base URL、模型选择）已就绪
- **启用步骤**：需要在 POM 中取消注释对应依赖并完成相关配置

**章节来源**
- [backend/src/main/java/com/bokagent/service/LLMService.java:13-30](file://backend/src/main/java/com/bokagent/service/LLMService.java#L13-L30)
...
BokAgentApplication -.-> LLMService : "可选依赖"
```

**图表来源**
- [backend/src/main/java/com/bokagent/service/LLMService.java:14-73](file://backend/src/main/java/com/bokagent/service/LLMService.java#L14-L73)
...
Note over LLM : "当前返回降级信息"
LLM-->>App : "返回：LLM服务暂未启用..."
App-->>Client : "返回降级响应"
```

**图表来源**
- [backend/src/main/java/com/bokagent/service/LLMService.java:27-50](file://backend/src/main/java/com/bokagent/service/LLMService.java#L27-L50)
...
- LLM 调用失败
  - **当前状态**：LLM 服务返回降级信息，这是预期行为
  - 如需启用 LLM 功能，检查 POM 中 Spring AI 依赖是否已启用
  - 确认 application.yml 中的 API Key 与 Base URL 配置正确
  - 检查网络连通性和外部服务可用性
- Actuator 健康检查
  - 通过 /actuator/health 验证服务状态
- 依赖启用问题
  - 确认 Spring Milestones 仓库配置正确
  - 检查 LangGraph4J 仓库配置（如需启用）
  - 验证依赖版本兼容性

**章节来源**
- [backend/src/main/java/com/bokagent/service/LLMService.java:28-30](file://backend/src/main/java/com/bokagent/service/LLMService.java#L28-L30)
...
## 结论
本项目通过 Spring Boot 3.5 与 Spring AI 1.1 的组合，结合 MyBatis-Plus 3.5、LangGraph4J、MinIO 等关键依赖，构建了面向企业级的 AI Agent 工作流编排系统。POM 采用 BOM 管理与 dependencyManagement 统一版本，配合合理的依赖范围与构建插件，确保了可维护性与可扩展性。容器化部署进一步强化了跨环境一致性与 UTF-8 支持。

**当前状态**：LLM 功能由于 Spring AI 依赖被注释而处于非功能性状态，系统已实现优雅降级，不影响基础工作流执行功能。建议在完成依赖配置后启用 LLM 功能，并持续关注依赖安全与版本升级策略，保持生态同步。
...
### 依赖启用指南
当准备启用 Spring AI 和 LangGraph4J 功能时，请按照以下步骤操作：

1. **启用 Spring AI 依赖**：
   - 在 POM 中取消注释 spring-ai-openai-spring-boot-starter 依赖
   - 确保 Spring Milestones 仓库可用
   - 验证 dependencyManagement 中的 Spring AI BOM 配置

2. **启用 LangGraph4J 依赖**：
   - 在 POM 中取消注释 langgraph4j-core 依赖
   - 添加必要的自定义仓库配置
   - 验证版本兼容性

3. **配置验证**：
   - 检查 application.yml 中的 API Key 和 Base URL
   - 测试 LLMService 的 chat 方法
   - 验证工作流引擎的完整功能

4. **监控与日志**：
   - 查看 Spring AI 相关的日志级别设置
   - 监控外部服务的调用性能和错误率
