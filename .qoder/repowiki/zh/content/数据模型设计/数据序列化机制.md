...
## 更新摘要
**变更内容**
- 更新JsonbTypeHandler实现，移除PostgreSQL特定的PGobject依赖
- 采用更通用的JSON字符串处理方式，提升数据库兼容性
- 更新架构图和实现细节以反映新的序列化策略
...
## 核心组件
- JsonbTypeHandler：基于Jackson的自定义MyBatis类型处理器，负责将复杂Java对象序列化为JSON字符串写入数据库，并在读取时反序列化回Java对象。**更新**：移除了PostgreSQL特定的PGobject依赖，采用更通用的字符串处理方式。
...
**章节来源**
- [JsonbTypeHandler.java:1-63](file://backend/src/main/java/com/bokagent/handler/JsonbTypeHandler.java#L1-L63)
- [Workflow.java:1-32](file://backend/src/main/java/com/bokagent/entity/Workflow.java#L1-L32)
- [GraphData.java:1-15](file://backend/src/main/java/com/bokagent/entity/GraphData.java#L1-L15)
- [Node.java:1-15](file://backend/src/main/java/com/bokagent/entity/Node.java#L1-L15)
- [NodeData.java:1-15](file://backend/src/main/java/com/bokagent/entity/NodeData.java#L1-L15)
...
- 设计要点
  ...
  - **更新**：采用通用JSON字符串处理方式，直接使用ps.setString()设置参数，不再依赖PostgreSQL特定的PGobject
  - 读取时从ResultSet/CallableStatement中获取字符串，再反序列化为目标类型
  - 对空值与异常进行显式处理，抛出SQLException以便MyBatis统一捕获

- 关键方法与行为
  - setNonNullParameter：序列化为JSON字符串并设置为字符串参数
...
## 结论
BokAgent的序列化机制以JsonbTypeHandler为核心，结合Jackson与MyBatis-Plus实现了复杂对象到JSON的可靠持久化。**更新**：通过移除PostgreSQL特定的PGobject依赖，采用更通用的JSON字符串处理方式，显著提升了数据库兼容性，使得系统能够更好地支持PostgreSQL和MySQL。通过Flyway管理数据库迁移，配合合理的Jackson与MyBatis配置，系统在不同数据库环境下都具备良好的性能与可维护性。
